...
def roll_dice(num_rolls, dice=six_sided):
    ...
    total = 0
    while num_rolls > 0:
        this_turn_total = dice()
        if this_turn_total == 1:
            return 1
        total += this_turn_total
        num_rolls -= 1
    return total
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    high_total= 0
    high_dice = 0
    func1 = make_averaged(dice, 1000)
    for x in range(1, 11):
        total = make_averaged(roll_dice, 1000)(x, dice)
        if total > high_total:
            high_total = total
            high_dice = x
        print(x, "dice scores", total, "on average")
    return high_dice
...
def run_experiments():
    ...
    if True: # Change to True to test final_strategy
        ...
        for x in range (1, 101):
            total += average_win_rate(final_strategy)
        print('final_strategy win rate:', total / 100)
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if take_turn(0, opponent_score) + score > 99:
        return 0
    if swap_strategy(score, opponent_score) == 0:
        return 0
    if bacon_strategy(score, opponent_score) == 0:
        return 0   
    if ((take_turn(0, opponent_score) + score) + opponent_score) % 7 == 0:
        return 0
    if score > 89:    
        return 0
    if score > 79:
